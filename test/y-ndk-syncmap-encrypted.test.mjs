...
export const testSyncMapEncrypted = async (tc) => {
  ...
  const encryptToReceivers = (input) => {
    const encryptedInput = box.encrypt(
      Buffer.from(input),
      [alice, bob].map((key) => key.publicKey),
    );
  ...
  const senderAlicePublicNostrKeyHex = getPublicKey(
    senderAliceSecretNostrKeyBytes,
  );
  ...
  const eveReceiveShouldnotupdate = eveYdoc
    .getMap("test")
    .get("contents")
    .toJSON();
  await testing.compare(
    eveReceiveShouldnotupdate,
    plaintext,
    "objects are equal",
  );
};
